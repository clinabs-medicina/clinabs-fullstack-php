...
if(isset($ag->data_agendamento)) {
    if ($data->event == 'PAYMENT_CONFIRMED' && $data->payment->billingType == 'CREDIT_CARD' || $data->payment->billingType == 'DEBIT_CARD' ) {
        ...
        $data_consulta = strtotime($ag->data_agendamento);
        $now = strtotime(date('Y-m-d H:i:s'));
        

        if(floor(($data_consulta - $now) / 3600) >= 24) { // 24hs antes da consulta
            $date_schedule = date('Y-m-d H:i:s', strtotime($ag->data_agendamento) - 86400);
            $wmsg = $msg .= "Você tem 1h para poder realizar o Pagamento, caso contrário a consulta será cancelada automaticamente.";
            $cancel_payment = date('Y-m-d H:i:s', strtotime($ag->data_agendamento) - 86400 + 3600 + 60);
        } else if(floor(($data_consulta - $now) / 3600) >= 1) { // 1h antes da consulta
            $date_schedule = date('Y-m-d H:i:s', strtotime($ag->data_agendamento) - 3600);
            $wmsg .= "Você tem 15min para poder realizar o Pagamento, caso contrário a consulta será cancelada automaticamente.";
            $cancel_payment = date('Y-m-d H:i:s', $data_consulta - 3600 + 900 + 60);
        } else if(($data_consulta - $now) < 3600){

            $date_schedule = date('Y-m-d H:i:s', $now + 60);
            $wmsg .= "Você tem 15min para poder realizar o Pagamento, caso contrário a consulta será cancelada automaticamente.";
            $cancel_payment = date('Y-m-d H:i:s', $now + 950 + 60);
        }
        ...
        $msg .= "Link de Pagamento: {$data->payment->invoiceUrl}" . PHP_EOL;
        $msg .= $wmsg;
        ...
        $payload = [
            ...
            'calc' => [
                ...
                'now' => $now,
                'reduce' => floor(($data_consulta - $now) / 3600),
                'scheduled' => strtotime($date_schedule)
            ],
            'data_scheduled' => $date_schedule,
            'data_cancel' => $cancel_payment,
            'msg' => $wmsg,
            'ag' => $ag
        ];


        $pdo->query("INSERT INTO `CRONTAB` (`nome`, `data`, `message`, `celular`, `type`, `status`, `link`, `agenda_token`, `payment_id`) VALUES ('LEMBRETE DE PAGAMENTO', '{$date_schedule}', '{$msg}', '{$ag->paciente_celular}', 'AGENDA_MED', 'PENDENTE', 'https://clinabs.com/', '{$ag->token}', '{$data->payment->id}');");
        $pdo->query("INSERT INTO `CRONTAB` (`nome`, `data`, `message`, `celular`, `type`, `status`, `link`, `agenda_token`, `payment_id`) VALUES ('CANCELAR PAGAMENTO', '{$cancel_payment}', '', '{$ag->paciente_celular}', 'DELETE_PAYMENT', 'PENDENTE', 'https://clinabs.com/', '{$ag->token}', '{$data->payment->id}');");
    }
}
 else {
    $payload = [
        'status' => 'error',
        'text' => 'Pagamento/Paciente não Encontrado'
    ];
 }
