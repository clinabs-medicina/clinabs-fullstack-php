...
class WhatsApp
{
    ...
    public function requestProfileImage($phoneNumber) {
        ...
        file_put_contents($_SERVER['DOCUMENT_ROOT'].'/data/logs/WhatsApp/WA_'.date('d_m_Y_H_i').'_'.uniqid().'.json', json_encode(json_decode($response), JSON_PRETTY_PRINT));
    ...
    public function sendDocMessage($phoneNumber, $text, $docUrl, $linkTitle = '') {
                ...
                $response = curl_exec($curl);
                file_put_contents('wa-receita.txt', $response);
                file_put_contents('wa-receita.txt', $docUrl, FILE_APPEND);
                curl_close($curl);
