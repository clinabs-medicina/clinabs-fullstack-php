...
(function (global, factory) {
   ...
   */
    ...
    const getElement = object => { // it's a jQuery object or a node element
        if (isElement$1(object)) {
            return object.jquery ? object[0] : object;
        }

        if (typeof object === 'string' && object.length > 0) {
            return document.querySelector(object) ? document.querySelector(object) : null;
        }
   ...
   */
    ...
    const onDOMContentLoaded = callback => {
        if (document.readyState === 'loading') { // add listener on the first call when the document is in loading state
   ...
   */
    ...
    function getTypeEvent(event) { // allow to get the native events from namespaced events ('click.bs.button' --> 'click')
    ...
    const EventHandler = {
        ...
        off(element, originalTypeEvent, handler, delegationFunction) {
            ...
            if (typeof callable !== 'undefined') { // Simplest case: handler is passed, remove that listener ONLY.
   ...
   */
    ...
    const Manipulator = {
        setDataAttribute(element, key, value) {
            element.setAttribute(`data-bs-${
                normalizeDataKey(key)
            }`, value);
        },

        removeDataAttribute(element, key) {
            element.removeAttribute(`data-bs-${
                normalizeDataKey(key)
            }`);
        },
   ...
   */
    ...
    function isShadowRoot(node) { // IE 11 has no ShadowRoot
    ...
    function contains(parent, child) {
        ...
        if (parent.contains(child)) {
            return true;
        } // then fallback to custom implementation with Shadow DOM support else if (rootNode && isShadowRoot(rootNode)) {
            var next = child;
    ...
    function getContainingBlock(element) {
        ...
        if (isIE && isHTMLElement(element)) { // In IE 9, 10 and 11 fixed elements containing block is always established by the viewport
    ...
    function mapToStyles(_ref2) {
        ...
        var _ref4 = roundOffsets === true ? roundOffsetsByDPR({x: x, y: y}) : {
            x: x,
            y: y
        };
    ...
    function getScrollParent(node) {
        if (['html', 'body', '#document'].indexOf(getNodeName(node)) >= 0) { // $FlowFixMe[incompatible-return]: assume body is always available
  ...
  */
    ...
    function computeOffsets(_ref) {
        ...
        if (mainAxis != null) {
            ...
            switch (variation) {
                case start: offsets[mainAxis] = offsets[mainAxis] - (reference[len] / 2 - element[len] / 2);
                    break;
    ...
    function flip(_ref) {
        ...
        if (makeFallbackChecks) { // `2` may be desired in some cases – research later
    ...
    function orderModifiers(modifiers) { // order based on dependencies
    ...
    var createPopper$2 = /*#__PURE__*/
    popperGenerator(); // eslint-disable-next-line import/no-unused-modules

    var defaultModifiers$1 = [eventListeners, popperOffsets$1, computeStyles$1, applyStyles$1];
    var createPopper$1 = /*#__PURE__*/
    popperGenerator({defaultModifiers: defaultModifiers$1}); // eslint-disable-next-line import/no-unused-modules
    ...
    popperGenerator({defaultModifiers: defaultModifiers}); // eslint-disable-next-line import/no-unused-modules
   ...
   */
    ...
    class Backdrop extends Config {
        ...
        _configAfterMerge(config) { // use getElement() with the default "body" to get a fresh Element on each instantiation
   ...
   */
    ...
    class Modal extends BaseComponent {
        ...
        _showElement(relatedTarget) { // try to append dynamic modal
        ...
        _addEventListeners() {
            ...
            EventHandler.on(this._element, EVENT_MOUSEDOWN_DISMISS, event => { // a bad trick to segregate clicks that may start inside dialog but end outside, and avoid listen to scrollbar clicks
   ...
   */
    ...
    EventHandler.on(document, EVENT_CLICK_DATA_API$1, SELECTOR_DATA_TOGGLE$1, function (event) {
        ...
        EventHandler.one(target, EVENT_HIDDEN$3, () => { // focus on trigger when it is closed
            if (isVisible(this)) {
                this.focus();
            }
   ...
   */
    ...
    class ScrollSpy extends BaseComponent {
        ...
        _initializeTargetsAndObservables() {
            ...
            for (const anchor of targetLinks) { // ensure that the anchor has an id and is not disabled
        ...
        _activateParents(target) { // Activate dropdown parents
