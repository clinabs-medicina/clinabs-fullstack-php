let tabControl = function (e) {
        ...
        e.removeEventListener("click", function () { }),
            e.addEventListener("click", function (e) {
                ...
                n.forEach(e => {
                    ...
                    $(`.tab[data-tab="${t}"][data-index="${e.target.dataset.index
                        }"]`).addClass("active")
            })
    })
},
    Slider = function (e, t = 3e3) {
        ...
        for (let f = 0; f < n.length; f++) {
            ...
            p.setAttribute("id", `slider${f + 1
                }`),
                p.setAttribute("type", "radio"),
                p.setAttribute("name", "btn-radio");
            let h = document.createElement("div");
            h.setAttribute("class", `auto-btn${f + 1
                }`),
                $(o).append(p),
                $(a).append(m),
                $(s).append(h)
...
let wizardModal = function (e, t = {
    ...
    $(r[0]).addClass("active"),
        ...
        $(`${e} .form-footer button[name]`).on("click", function (n) {
            ...
            if (t.validate && "btn-step-prev" !== this.name) {
                if ($(r[o]).find("*[required]").each(function () {
                    0 === this.value.length ? ($(this).addClass("input-error"), "select" == this.tagName ? ($(`span[aria-controls="select2-${this.id
                        }-container]`).addClass("input-error"), d.push($(`span[aria-controls="select2-${this.id
                            }-container]`)), t.onValidate($(`span[aria-controls="select2-${this.id
        ...
        t.autoFill && ($(a).find("input").each(function () {
            null !== localStorage.getItem(this.name) && ("file" == this.type ? $(`input[name="${this.id
                }"]`).val(localStorage.getItem(this.name)) : this.value = localStorage.getItem(this.name))
        }), $(a).find("select").each(function () {
            null !== localStorage.getItem(this.name) && $(this).find(`option[value=${localStorage.getItem(this.name)
    ...
    switch (t) {
        ...
        case "agenda-accept":
            Swal.fire({
                ...
                didOpen: function () {
                    $.getJSON(`/forms/agenda.actions.php?paciente_token=${$(e).data("paciente")
                        }&action=${t}&token=${n}&ref=${$(e).data("status")
                        }`, function (t) {
                            if ("success" === t.status) {
                                let a = $(`#${n}`);
                ...
                Swal.fire({
                    ...
                    didOpen: function () {
                        $.getJSON(`/api/pix/index.php?valor=${s}&identificador=5656565&descricao=CONSULTA MEDICA CLINABS`, function (e) {
                            Swal.fire({
                                confirmButtonText: "Fechar Janela", imageUrl: "/api/pix/logo_pix.png", html: `<p>Use seu App de Pagamento para Escanear o QrCode para Pagamento</p><img height="256px" src="${e.imageString
                                    }" alt=""><p><b>Valor:</b>  R$ ${e.valor.toLocaleString("pt-br", {
                                        style: "currency",
                                        currency: "BRL"
                                    })
                                    }<p><b>Beneficiario:</b> ${e.beneficiario
                                    }`
                            })
                        })
                    },
        ...
        $("#form-login-secure").on("submit", function (e) {
            e.preventDefault(),
                ...
                Swal.fire({
                    ...
                    allowOutsideClick: !1,
                    didOpen: function () {
                        $.get(`/forms/login.form.php?usuario=${$("#user-login").val()
        ...
        $("#form-login-recovery-secure").on("submit", function (e) {
            e.preventDefault(),
                ...
                Swal.fire({
                    ...
                    allowOutsideClick: !1,
                    didOpen: function () {
                        $.post(`/forms/login.form.recovery.php?token=${$('input[name="token"]').val()
                            }&usuario=${$("#user-login").val() || $("#token").val()
                            }&password=${$("#user-password").val()
...
function addAddress() {
    if (0 === $(".street-editor").find("[name][required]").filter(function () {
        ...
        <div class="street-item" id="${n}">
            ...
            <input type="hidden" name="enderecos[]" data-token="${n}" value="${JSON.stringify(e).replace(/"/g, "'")
                }">
    </div>`;
        ...
        $(".street-editor").find("[name]").filter(function () {
            return 0 === this.value.length
        })[0].focus()
                                        ...
                                        if ("error" in e)
                                            ...
                                            $("#cert_common_name").val(e.name),
                                                ...
                                                e.name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/^\s+|\s+$/gm, "").trim() === t.replace(/^\s+|\s+$/gm, "").trim() ? Swal.close() : Swal.fire({
                                                    ...
                                                    icon: "warning",
                                                    allowOutSideClick: !1,
                                                    html: `Esta Assinatura Digital n\xe3o pode Ser Utilizada por <b>${t}</b>! <br><br>Esta Assinatura Pertence a <b style="color: red !important"><p>${e.name
                                                        }</p>.</b>`
                                                }).then(function () {
                                                    window.location.reload()
                                                })
