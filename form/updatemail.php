...
if(isset($_GET['nome']) && isset($_GET['mail']) && isset($_GET['password'])) {
    ...
    if(isset($_GET['mailpwd']) && !empty($_GET['mailpwd'])) {
            $stmt = $pdo->prepare('UPDATE `CONTAS_EMAIL` SET nome = :nome, senha = :senha WHERE email = :email');
            ...
            try {
                ...
                $json = json_encode(['status' => 'error','icon' => 'info', 'text' => 'Não foi Possível Adicionar Esta Conta. Talvez já Existe uma Conta como Essa.']);
            }
    } else {
        $stmt = $pdo->prepare('UPDATE `CONTAS_EMAIL` SET nome = :nome WHERE email = :email');
        $stmt->bindValue(':nome', strtoupper($_GET['nome']));
        $stmt->bindValue(':email', strtolower($_GET['mail']));
        ...
        try {
            ...
            $mail_accounts = $pdo->query('SELECT * FROM CONTAS_EMAIL');
                ...
                foreach($mail_accounts->fetchAll(PDO::FETCH_OBJ) as $email){
                    file_put_contents($file, "{$email->email}|{SHA512-CRYPT}{$email->senha}\n", FILE_APPEND);
                }
