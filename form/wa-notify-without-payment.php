...
try {
    $xstmt = $pdo->query("SELECT payment_id  FROM VENDAS WHERE reference = '{$_GET['token']}'");
    ...
    if(strtotime($paciente->data_agendamento) > time()) {
        $data = date('d/m/Y', strtotime($paciente->data_agendamento)).' às '.date('H:i', strtotime($paciente->data_agendamento));
    } else {
        $data = date('d/m/Y', strtotime($paciente->data_cancelamento)).' às '.date('H:i', strtotime($paciente->data_cancelamento));
    }
    ...
    if($payment->status == 'PENDING') {
        ...
        $msg .= "".PHP_EOL;
        $msg .= "Caso deseja pagar na Hora da Consulta, desconsidere este aviso.";
        $msg .= "".PHP_EOL;
        
        if($payment->billingType == 'PIX') {
            $msg .= "".PHP_EOL;
            $msg .= "*Você vai receber o código PIX.*".PHP_EOL;
            $msg .= "*Você pode copiar e colar o código para pagamento.*".PHP_EOL;
        }
        ...
        $dta = date('d/m/Y', strtotime($paciente->data_agendamento));
        $hr = date('H:i', strtotime($paciente->data_agendamento));

        $msg = "Olá *{$prefixo}* *{$paciente->medico_nome}*".PHP_EOL;
        $msg .= "Você tem uma Consulta Agendada com o Paciente *{$paciente->nome_completo}*".PHP_EOL;
        $msg .= "em {$dta} {$hr}".PHP_EOL;
        $msg .= "Contato: {$payment->mobilePhone}".PHP_EOL;

        if($ag->modalidade == 'ONLINE') {
            $meetlink = json_decode($ag->meet)->hostRoomUrl;
        }

        if(trim($asaas->get_status($payment->status)) == 'AGENDADO') {
            $wa->sendTextMessage(
                phoneNumber: $paciente->medico_celular,
                text: $msg
            );
        }
